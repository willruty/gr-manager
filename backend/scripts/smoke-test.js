...
async function main() {
    ...
    if (createdCliente?.id)
        createdIds.cliente = createdCliente.id;
    ...
    if (createdFunc?.id)
        createdIds.funcionario = createdFunc.id;
    ...
    if (createdIds.cliente) {
        ...
        if (createdContrato?.id)
            createdIds.contrato = createdContrato.id;
    }
    ...
    if (createdIds.funcionario)
        docBody.funcionario_id = createdIds.funcionario;
    const { data: createdDoc } = await test("Criar documento", "POST", "/documentos", docBody);
    if (createdDoc?.id)
        createdIds.documento = createdDoc.id;
